export default class SliderInput {
  constructor(element, callback, xPad, yMin, yMax, min, max, radius) {
  ...
  startTouch(event) {
    ...
    const offset = this.offsetPoints(
  ...
  offsetPoints(x, y, event) {
    ...
    return {
      x: (x - targetRect.left) * scaleX,
      y: (y - targetRect.top) * scaleY
    };
  }

  determineValue(event) {
    event.preventDefault();
    event.stopPropagation();

    if (
      this.xStart === null ||
      this.xStart < this.xPad - this.radius ||
      this.xStart > this.maxWidth - this.xPad + this.radius ||
      this.yStart < this.yMin ||
      this.yStart > this.yMax
    ) {
      return;
    }
    ...
    const value = Math.max(
      ...
      Math.min(
        Math.round(
          ((xEnd - this.xPad) * (this.max - this.min)) /
            (this.maxWidth - this.xPad * 2) +
            this.min
        ),
        this.max
      )
    ...
    if (typeof this.callback === "function") {
      const x = Math.max(
        this.xPad,
        Math.min(xEnd, this.maxWidth - this.xPad)
      );
  ...
  setValue(event) {
    ...
    if (
      ...
      const zeroCode = keyCode <= 57 ? 48 : 96;
      const number = keyCode - zeroCode;
    ...
    const x =
      this.min === this.max
        ? this.maxWidth / 2
        : (Math.max(this.value, this.min) - this.min) *
            ((this.maxWidth - this.xPad * 2) /
              (this.max - this.min)) +
          this.xPad;
  ...
  trigger(value) {
    ...
    const x =
      this.min === this.max
        ? this.maxWidth / 2
        : (Math.max(value, this.min) - this.min) *
            ((this.maxWidth - this.xPad * 2) /
              (this.max - this.min)) +
          this.xPad;
